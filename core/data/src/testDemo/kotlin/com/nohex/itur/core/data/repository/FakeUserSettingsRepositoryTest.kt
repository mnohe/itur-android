...
class FakeUserSettingsRepositoryTest {
    ...
    @Test
    fun `GIVEN a new repository WHEN setting an email THEN settings emits the new email`() =
        runBlocking {
            val repo = repository()
    ...
    @Test
    fun `GIVEN an email is set WHEN setting a different email THEN settings emits the latest email`() =
        runBlocking {
            val repo = repository()
