...
class FakeActivityRepositoryTest {
    ...
    private fun repository(vararg activities: IturActivity) =
        FakeActivityRepository(initialActivities = activities.toList())

    // --- getActivity ---

    @Test
    fun `GIVEN a seeded activity WHEN getting it by ID THEN returns Success with the activity`() =
        runBlocking {
    ...
    @Test
    fun `GIVEN activities WHEN filtering by organizer THEN returns only their activities`() =
        runBlocking {
    ...
    @Test
    fun `GIVEN mixed-status activities WHEN filtering ongoing by organizer THEN returns only ongoing`() =
        runBlocking {
    ...
    @Test
    fun `WHEN creating an activity THEN returns Success with a valid 20-char alphanumeric ID`() =
        runBlocking {
    ...
    @Test
    fun `GIVEN an activity WHEN adding a participant THEN they appear in the participant list`() =
        runBlocking {
    ...
    @Test
    fun `GIVEN no matching activity WHEN adding a participant THEN throws IllegalArgumentException`() =
        runBlocking {
            try {
    ...
    @Test
    fun `GIVEN an activity WHEN removing a participant THEN they are absent from the participant list`() =
        runBlocking {
    ...
    @Test
    fun `GIVEN an activity WHEN updating its status THEN the new status is reflected`() =
        runBlocking {
    ...
    @Test
    fun `GIVEN an activity WHEN deleting it THEN returns Success with the deleted activity`() =
        runBlocking {
